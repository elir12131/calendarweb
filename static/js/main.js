...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const topScrollbar = document.querySelector('.top-scrollbar-container');
    const topScrollbarThumb = document.querySelector('.top-scrollbar-thumb');
    ...
    function setupTopScrollbar() {
        ...
        const syncScroll = () => {
            ...
            topScrollbar.style.display = isScrollable ? 'block' : 'none';
            if (!isScrollable) return;
            const scrollableWidth = scrollWrapper.scrollWidth - scrollWrapper.clientWidth;
            const thumbWidth = (scrollWrapper.clientWidth / scrollWrapper.scrollWidth) * 100;
            topScrollbarThumb.style.width = `${thumbWidth}%`;
            const scrollPercentage = scrollWrapper.scrollLeft / scrollableWidth;
            const thumbPosition = scrollPercentage * (100 - thumbWidth);
            topScrollbarThumb.style.left = `${thumbPosition}%`;
        };
        scrollWrapper.addEventListener('scroll', () => { if (!isDragging) syncScroll(); });
        topScrollbarThumb.addEventListener('mousedown', (e) => {
            isDragging = true;
            topScrollbarThumb.classList.add('active');
            const startX = e.pageX;
            const startScrollLeft = scrollWrapper.scrollLeft;
            const onMouseMove = (moveEvent) => {
                if (!isDragging) return;
                const dx = moveEvent.pageX - startX;
                const scrollRatio = (scrollWrapper.scrollWidth - scrollWrapper.clientWidth) / (topScrollbar.clientWidth - topScrollbarThumb.offsetWidth);
                scrollWrapper.scrollLeft = startScrollLeft + dx * scrollRatio;
                syncScroll();
            };
            const onMouseUp = () => {
                isDragging = false;
                topScrollbarThumb.classList.remove('active');
