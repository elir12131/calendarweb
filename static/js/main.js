...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- GLOBAL STATE ---
    let calendarImage = null; // This will store the generated screenshot
    let currentZoom = 1.0;
    const zoomStep = 0.2;
    ...
    async function fetchAndRenderAll() {
        ...
        try {
            ...
            // Render the full grid into the hidden container to be screenshotted
            hiddenRenderContainer.innerHTML = '';
            hiddenRenderContainer.appendChild(createCalendarGrid(calendarData));
            
            // Generate the single screenshot
    ...
    async function generateCalendarImage() {
        try {
            const canvas = await html2canvas(hiddenRenderContainer.querySelector('.calendar-grid'), {
                width: 1200,
                windowWidth: 1200,
    ...
    function addSampleBirthday() { if (sampleBirthdayCount >= 4) return; const availableLists = Array.from(document.querySelectorAll('#hidden-render-container .events-list')).filter(list => !list.querySelector('.sample-birthday')); if (availableLists.length === 0) return; const randomList = availableLists[Math.floor(Math.random() * availableLists.length)]; const birthdayEl = document.createElement('div'); birthdayEl.className = 'sample-birthday'; birthdayEl.textContent = 'Happy Birthday!'; randomList.appendChild(birthdayEl); sampleBirthdayCount++; if (sampleBirthdayCount >= 4) { addBirthdayBtn.disabled = true; addBirthdayBtn.textContent = "Max Samples Added"; } generateCalendarImage().then(handleResize); }
    ...
    function handleResize() {
        ...
        if (window.innerWidth <= 991) {
            // Mobile View: Show the screenshot preview
            calendarContainer.className = 'mobile-preview';
            if (calendarImage) {
                const img = document.createElement('img');
                img.src = calendarImage;
            ...
            // Desktop View: Render the interactive HTML calendar
            calendarContainer.className = '';
            hiddenRenderContainer.innerHTML = '';
            calendarContainer.appendChild(createCalendarGrid(fullCalendarData));
        }
    }

    function applyZoom() { modalContent.style.transform = `scale(${currentZoom})`; }
    function zoomIn() { currentZoom += zoomStep; applyZoom(); }
    function zoomOut() { currentZoom = Math.max(0.2, currentZoom - zoomStep); applyZoom(); }
    ...
    let resizeTimeout;
    window.addEventListener('resize', () => { clearTimeout(resizeTimeout); resizeTimeout = setTimeout(handleResize, 250); });
