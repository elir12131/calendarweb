...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let screenshotUrl = null; // Store the URL of the generated image
    ...
    async function fetchAndRenderAll() {
        ...
        try {
            ...
            // Render the full-size calendar into the hidden container first.
            renderCalendar(fullCalendarData, hiddenRenderContainer);
            // Then, take the screenshot and place it in the visible container.
    ...
    function generatePreviewScreenshot() {
        const target = hiddenRenderContainer.querySelector('.calendar-grid');
        if (!target) return;
        
        html2canvas(target, { scale: 1 }).then(canvas => {
            screenshotUrl = canvas.toDataURL('image/jpeg', 0.9);
            const img = document.createElement('img');
            img.src = screenshotUrl;
            img.className = 'preview-screenshot';
            
            // Clear spinner and add the screenshot and button to the main container
            calendarContainer.innerHTML = ''; 
            calendarContainer.appendChild(img);
            
            const previewButton = document.createElement('div');
            previewButton.className = 'preview-button';
            previewButton.textContent = 'Tap to Preview';
            calendarContainer.appendChild(previewButton);
        });
    }
    ...
    // This logic toggles the .mobile-preview class on the main container
    function handleResize() {
        if (window.innerWidth <= 991) {
            calendarContainer.classList.add('mobile-preview');
        } else {
            // On desktop, we want the actual calendar, not the screenshot
